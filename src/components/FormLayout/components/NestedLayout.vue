...
import TopBottomLayout from './TopBottomLayout.vue'

const props = withDefaults(defineProps<NestedLayoutProps>(), {
  topCollapsed: false,
  leftCollapsed: false,
  topCollapsible: true,
...
function onLeave(el: Element, done: () => void): void {
  gsap.to(el, { width: 0, opacity: 0, overflow: 'hidden', duration: 0.26, ease: 'power2.inOut', onComplete: done })
}
</script>

<template>
  <div class="nested-layout flex flex-row items-stretch w-full h-full min-h-0 relative">
    ...
    <div v-if="leftCollapsible" class="nested-divider h-full w-[10px] shrink-0 flex items-center">
      <CollapseTrigger
        variant="divider"
    ...
    <div class="right-nested-area flex-1 min-w-0 h-full min-h-0">
      <TopBottomLayout
        :top-collapsed="topCollapsed"
        :top-collapsible="topCollapsible"
        :top-title="topTitle"
        :top-icon="topIcon"
        :card="card"
        @toggle-top="emitToggleTop"
      >
